...
if [[ -z "${ACC_FOLDER}" ]]; then
	...
	#there are several problems with this in the current implementation  
	#The start time is currently the ACC_TIME - 511 seconds
	#the duration is ACC_DUR - 7 seconds
	#This may be fixed with a patch to iLiSA or will be recalculated here
	#TODO: Fix this file creation routine
	ACC_FILE="${stnID}_${ACC_DATE}T${ACC_TIME}_${ACC_RCU}_${ACC_SRC}_${ACC_DUR}_ct20171108_acc2bst.hdf5"
	...
	#calls beamModelTester's comparison module in interactive mode
	${BMT_DIR}/comparison_module/comparison_module_1_0.py --model ${CSV_FILE} --scope ${HDF5_FILE} --interactive 2 --verbose 2
